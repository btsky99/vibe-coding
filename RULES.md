...
## 🧠 1. 지능형 하이브 지식 공유 (Memory-First)
1. **작업 시작 전 지식 로드 (Mandatory)**: 코드를 수정하기 전, 다음 데이터를 반드시 순차적으로 확인하여 하이브의 최신 컨텍스트를 동기화해야 합니다.
   - **`memory.md` (필독)**: 프로젝트의 장기 기억, 기술적 결정 사항, 사용자 선호도가 담긴 파일입니다.
   - **`python scripts/memory.py list`**: 현재 세션의 즉각적인 공유 지식을 로드합니다.
   - **`python scripts/orchestrator.py --summary`**: 하이브의 실시간 상태(에이전트 현황, 태스크 부하 등)를 브리핑받습니다.
...
3. **지식 기록 및 학습 (Intelligence Learning)**:
   - **[Claude 4.6+ 특화]** 작업 중 발견한 라이브러리 특성, 버그 해결책, 프로젝트 고유 패턴은 즉시 **`memory.md`**의 관련 섹션에 스스로 기록하십시오.
   - "이걸 기억해줘"라는 사용자 요청이 없더라도, AI가 판단하기에 중요한 결정 사항은 **장기 기억(`memory.md`)**에 남겨 다른 에이전트와 지식을 공유해야 합니다.
4. **상태 확인**: 작업 시작 전, 가급적 `.ai_monitor/data/task_logs.jsonl` 파일의 최근 내역을 읽어 다른 에이전트가 어떤 작업을 하고 있는지 확인하세요.
5. **마스터 지침**: 특별한 지시가 없다면 최신 오케스트레이터와 연동된 `vibe-master` 지침을 기반으로 행동합니다.
