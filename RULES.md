...
- 2026-05-26 Claude: 섹션 2 보강 — 파일당 1500줄 제한 + LLM 관점 주석 규칙 추가.
                    상세는 .claude/rules/file-limits.md 분리. 본 파일은 핵심 의무만 명시.
-->

# 📜 하이브 마인드 공통 행동 원칙 (Hive Mind Rules)

이 파일은 프로젝트에 참여하는 모든 AI 에이전트(Antigravity, Claude 등)가 반드시 준수해야 하는 **최우선 공통 규칙**입니다.

## 🧠 1. 지능형 하이브 지식 공유 (Postgres-First)
1. **작업 시작 전 지식 로드 (Mandatory)**: 코드를 수정하기 전, 다음 데이터를 반드시 확인하여 하이브의 최신 컨텍스트를 동기화해야 합니다.
...
## 📝 2. 파일 크기 제한 · LLM 관점 주석 · 중앙 프로젝트 맵

### 2-0. 파일당 1500줄 제한 (주석 포함) — **Mandatory**
- 모든 코드 파일은 주석 포함 **1500줄 이하**. `wc -l <file>` ≤ 1500.
- 초과 시 **즉시 도메인 단위로 분할**. "나중에 분리" 금지.
- 1200줄 도달 시 미리 분할 검토 시작.
- 분할 가이드 + 현재 위반 파일 목록: [`.claude/rules/file-limits.md`](.claude/rules/file-limits.md)

### 2-1. LLM 관점 주석 (사람용 자연어 풀어쓰기 금지) — **Mandatory**
주석의 독자는 사람이 아니라 **다음 세션의 LLM**입니다. 코드만 봐서는 알 수 없는 정보만 적습니다.

**❌ 금지**: "사용자를 가져온다", "결과 반환", "변수 초기화" — 코드를 자연어로 옮긴 주석
**✅ 필수**: WHY(의도) / 제약 조건 / 불변식 / 과거 사고 / 외부 의존 함정 / 호환성 가정

상세 예시 + 안티패턴: [`.claude/rules/file-limits.md`](.claude/rules/file-limits.md)

### 2-2. 표준 파일 헤더 — **Mandatory** (예외 없음)
LLM이 파일을 열기 전 검색·우선순위 판단용. 사람용 자연어가 아니므로 그대로 유지합니다.
...
### 2-3. 중앙 프로젝트 맵 (PROJECT_MAP.md)
- 개별 파일 단위의 `.md` 문서는 더 이상 생성하지 않습니다. `PROJECT_MAP.md`에 전체 구조와 핵심 역할을 최신 상태로 유지합니다.
- `docs/` 내에 코드와 1:1로 매칭되는 설명 문서 생성 금지 — 관리 포인트 분산 + 토큰 낭비.
- `PROJECT_MAP.md`도 너무 길어지면 토큰 낭비. 핵심 구조와 인터페이스 위주로 간결하게 유지.
