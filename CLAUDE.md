...
### 3단계: 자동 후처리
코드 수정 완료 후 자동 수행:
1. **문서 동기화 (Auto-Doc)**: 변경 사항에 맞게 `ai_monitor_plan.md` 진행 상태 최신화.
   - **(필수)** 수정한 소스 코드 파일의 맨 위 상단에 `docs/README.md` (전체 파일 리스트)와 `docs/해당파일명.md` (해당 파일의 개별 설명 문서) 링크를 한글 주석으로 반드시 달아줍니다.
   - **(필수)** `docs/` 폴더 내에 수정한 파일의 설명 문서를 업데이트 또는 신규 생성하고, `docs/README.md`에 목록을 추가합니다.
   - 모든 코드 주석과 커밋, 작업 내역은 반드시 **한글로 아주 자세하게** 작성해야 합니다.
