...
4. **한글 주석 및 문서화 의무 (Auto-Doc)**: 소스 코드를 수정할 때 **주석은 반드시 한글로 아주 상세하게** 작성해야 합니다. 또한, 각 파일 상단에 전체 문서 리스트(`docs/README.md`)와 해당 파일의 개별 상세 문서 링크를 주석으로 남기고, `docs/` 폴더 내의 문서들을 자동으로 생성 또는 업데이트(동기화)해야 합니다.
