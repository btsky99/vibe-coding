...
> **✅ 상태: Task 1~9 구현·출고 완료 (v3.7.243, 2026-07-04). 체크박스는 출고 반영 사후 갱신(2026-07-05).**
> **검증: 트랙 B(격리 함수 E2E) 18/18 PASS (2026-07-05). 트랙 A(진짜 설치본 배너→버튼→재시작 왕복) 미검증 — 최종 종결 조건.**
> 트랙 B 커버: 감지 ON/OFF·min_exe 게이트·🔴 로컬전용커밋 유실방지 가드(7/3 사고 방어막 실작동)·정상 apply(.soft_rollback+전진)·롤백 경로.

---

## 마일스톤 A — 부트스트랩 토대 (선행 필수)

### [x] Task 1: soft_manifest.json 생성 (풀빌드 게이트 규약)
...
### [x] Task 2: boot.py 신규 — 체크아웃 보장 + seed 폴백
...
### [x] Task 3: boot.py — EXE버전 게이트 + sys.path 주입 + runpy 라우팅
...
### [x] Task 4: soft_updater.py 신규 — SHA 감지
...
### [x] Task 5: soft_updater.py — 적용(apply) + 재시작
...
### [x] Task 6: server.py — soft-update 엔드포인트 2개 + 폴링 스레드
...
### [x] Task 8: vibe-coding.spec — entry 전환 + 의존성 hiddenimports 보강
- 파일: `vibe-coding.spec`
...
### [x] Task 9: build-release.yml CI 동기화 + 문서
