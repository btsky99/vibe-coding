...
[x] Task 15: 프로젝트별 보드   (의존: Task 14)   ✅ 2026-08-09 구현
    파일: apix-console/console/panels/projects.js (142줄)
    방법: 프로젝트별 카드 — 마지막 활동·진행 태스크·마지막 커밋·담당 노드.
          CipherTrader 처럼 git 이 없는 노드는 이벤트만으로 표시.
    검증: 5개 프로젝트가 전부 뜨고, 조용한 프로젝트는 조용하다고 표시된다.
    🔴 설계 판단: 조용함은 경보가 아니다. 활동이 뜸한 프로젝트에 노랑·빨강을 칠하면
       상시 경보가 되어 진짜 사건이 그 속에 묻힌다. 색은 심각도가 아니라 활동 여부다.
```

## Phase 8 — 릴리즈/CI + 사고 장부 — **방향 전환 (2026-08-09)**

원안은 '제품 릴리즈·CI 실패'를 서버 관제판에 넣는 것이었다. 그러나 바이브 코딩의
버전 배포는 그 제품 페이지에서 볼 일이지, **"서버가 지금 괜찮은가"를 보러 온 화면에
섞이면 정작 서버 사건이 묻힌다**. 관제판이 다루는 것은 서버에서 실제로 일어난
사건으로 한정하고, 제품 릴리즈는 각 제품 화면이 맡는다. (근거 커밋 e6ae341)

```
[—] Task 16: GitHub 폴러   ❌ 폐기 (위 방향 전환)
    폐기 사유: 제품 릴리즈는 서버 사건이 아니다. 화면 목적이 흐려진다.

[x] Task 17: 사고·교훈 패널 → **'서버 사건'으로 교체**   ✅ 2026-08-09 구현
    파일: apix-console/collector/watcher.py (189줄, 60초 주기 감시 스레드)
    기록 대상: 서비스 중단/복구 · 노드 연결/끊김/유령화 · 인증서 갱신 ·
              자원 임계 돌파/회복 · 인증 실패 급증
    🔴 상태 '값'이 아니라 '변화'만 남긴다 — 매 주기 현재값을 쓰면 하루 1440줄이
       쌓여 사건이 묻힌다.
    🔴 첫 스냅샷은 기록하지 않는다 — 재시작마다 가짜 사건이 쏟아지는 것을 막는다.
    🔴 노드가 죽을 때 '끊김'과 '유령화(포트만 열림)'를 구분한다 — 진단이 갈린다.
```

## Phase 9 — 노드 확산

```
[~] Task 18: 노드에 푸시 배포 — **PC 2대 완료, 나머지는 대상 자체가 정리됨**
    방법: 노드별 개별 토큰 발급. 노드마다 검증 후 다음으로.
    실측 (2026-08-11, `ssh apix … manage.py list`):
      · pc-yjscom (개발 PC) — 살아 있음
      · pc-na2js  — 살아 있음. SSH 는 거부되지만 하트비트는 정상이다.
        🔴 이 둘은 별개 통로다 — SSH 불가를 '노드 죽음'으로 읽지 말 것.
    원안의 나머지 대상은 실체가 바뀌었다:
      · CipherTrader 노드 = 개발 PC 자신 → 이미 포함(중복 등록 대상 아님)
      · 탭 S4 → 레노버로 이사하며 폐기
      · qeuhlak → 노드 실체 없음(node_seq 2번은 결번, 커밋 3ee2af1)
    🔴 남은 실대상은 lenovo 하나뿐이고 지금 붙일 수 없다 — ssh 별칭이 폐기된
       메시 VPN 주소(100.x)를 가리켜 접속이 타임아웃된다. 그 PC 앞에서
       Setup-ApixNode.ps1 을 1회 실행하거나 주소를 갱신하는 것이 선행 조건이다.
```
...
[x] Task 32: 신규 노드 온보딩 절차 — 키 발급→permitopen 등록→config 주입   ✅ 2026-08-11
              파일: scripts/remote/Setup-CentralNode.ps1 (224줄, 대상 PC 에서 1회 실행)
              검증: 임시 DataDir 로 실행 → 키 재사용 · config 주입(기존 키 보존) ·
                    **실제 VPS 터널 연결 성공**까지 확인.
              🔴 등록용 스크립트를 새로 만들지 않았다 — 서버 쪽 등록은 기존
                 vps-knowledge-db.sh 가 TUNNEL_PUBKEY 로 이미 처리(멱등)한다.
                 authorized_keys 옵션 문자열을 두 곳에 두면 한쪽만 고쳐지는 사고가 난다.
              🔴 개인키는 대상 PC 를 떠나지 않는다 → 2단계 구조가 불가피(2026-08-08 폐기 이력).
              🔴 .ps1 은 BOM 있는 UTF-8 로 저장할 것 — PS 5.1 이 BOM 없는 UTF-8 을
                 CP949 로 읽어 한글 주석이 깨지고 구문 오류가 난다(작성 중 실제로 밟음).
```
...
- `node_seq` 를 사람에게 묻는 것 — 번호는 이미 확정됐다(1=메인, **2=결번**, 3=na2js).
  🔴 2026-08-10 정정: 크립토(CipherTrader)는 별도 PC가 아니라 1번 개발 PC 자신이다.
  실재 노드는 2대뿐이고 2번을 채울 PC는 존재하지 않는다.
...
## 🎯 2026-08-12 요구 재정의 — 사용자가 원하는 것 5가지

계획을 이 다섯 줄에 대조해 다시 짰다. 각 Task 앞의 【N】은 어느 요구를 채우는지다.

| # | 요구 | 착수 전 상태 |
|---|---|---|
| 1 | btsky.pe.kr 에서 **언제 어디서든** 아픽스 비서를 부른다 | 재료 전부 존재(도메인·GitHub 로그인·PWA 자산·tmux), 조립만 |
| 2 | **음성**으로도 부른다 | 미착수. 브라우저 내장 API 면 무료·서버 부담 0 |
| 3 | 모든 PC 의 **프로젝트를 파악**하고 있다 | ✅ **이미 됨** — apix_projects + /api/projects + 하트비트가 목록을 나름 |
| 4 | 지시 하나로 **코드 수정부터 확인까지** | 절반 — 지시·수정 ✅ / 검수 ❌ / 세션 없을 때 기동 ❌ |
| 5 | 에이전트끼리 **서로 대화하며 자율 협업** | 배선 ✅ — 그러나 **우선순위를 낮춘다**(아래) |
| 6 | 각 PC 에 컨트롤 UI · **선택한 프로젝트만**(샌드박스) · 에이전트 **멀티 실행** | ✅ **대부분 이미 됨** — 게이트=샌드박스, 슬롯 T1~T8=멀티, 앱=컨트롤 UI |

### 🔴 요구 5 재정의 — **비서 경유가 기본, 직접 대화는 예외** (사용자 판단 2026-08-12)

사용자 판단: *"여기서 계속 컨트롤해야 한다는 거네? 서로 대화하는 것보다"* — 맞다.

| | 비서 경유(기본) | 에이전트끼리 직접 대화 |
|---|---|---|
| 종료 | 비서가 판단해 멈춘다 | **종료 조건이 없다** — 결론 없이 토큰만 탄다 |
| 기록 | 일감 하나에 모인다 | 대화가 흩어져 "어디서 틀어졌나"를 못 본다 |
| 개입 | 사람이 언제든 | 이미 저희끼리 진행된 뒤 |

2-1 이 1-3 에게 말을 걸면 실제로 꽂힌다(오늘 확인). 기술은 있으나 **쓰는 방식**을 정한다 —
직접 대화는 **한 일감 안에서 비서가 허락할 때만**. 그때도 목표·왕복 상한·TTL 을 걸고,
끝나거나 상한에 걸리면 사람에게 보고한다. 창당 상한(_MAX_REMOTE_PER_WINDOW)은 폭주
방지지 협업 설계가 아니다 — 그것만 믿으면 안 된다.

### 요구 6 — 샌드박스·멀티는 이미 서 있다

- **샌드박스** = 기동 게이트(`central_remote_launch.allow_dirs`). 허용 폴더와 그 하위만
  통과하고, 목록 밖은 needs_approval, `허용폴더/../다른곳` 같은 경로 눈속임은 차단된다
  (Task 50, 테스트로 고정). 지금 이 PC 는 `D:ibe-coding` 만 열려 있다.
- **멀티 실행** = 슬롯 T1~T8. 한 프로젝트에 claude 둘 + codex 하나도 주소(3-1·3-2·3-3)만
  다르면 된다. 새로 만들 것이 없다.
- **각 PC 컨트롤 UI** = 앱 자체가 그 역할이다(터미널 기동·중앙 수신·게이트 유지).
  board 여는 버튼(Task 68)만 얹으면 끝난다.
...
### 【1·4】 Task 55: 콘솔에 일감 API
    파일: /opt/apix/src/collector/app.py, .ai_monitor/src/pg_jobs.py (로직 공유)
    방법: GET /api/jobs, /api/jobs/detail, POST /api/jobs, /api/jobs/decide.
      응답 스키마를 로컬 jobs_api 와 동일하게 맞춘다 — 화면이 API_BASE 만 바꿔 끼우게.
    완료 조건: 브라우저에서 /api/jobs 가 오늘 만든 일감(#36·37)을 돌려준다

### 【1·3】 Task 56: board 화면 — 프로젝트와 일감을 한 화면에
    파일: /var/www/vibe/board/ (index.html, board.js, board.css)
    방법: 승인된 시안 구조 — 위 비서 조종석, 가운데 **프로젝트별 열**, 오른쪽 상세
      (무엇이 바뀌었나=git 실측 / 어디까지 갔나=전이 이력).
      프로젝트 목록은 **이미 있는** /api/projects 를 그대로 쓴다(요구 3 은 배선만).
      콘솔 스타일 계승, 바닐라. 기존 manifest·아이콘 재사용.
    완료 조건: 폰과 PC 에서 같은 화면이 뜨고, 모든 PC 의 프로젝트가 열로 보인다

### 【2】 Task 57: 음성 입출력 — 브라우저 내장으로
    파일: /var/www/vibe/board/voice.js
    방법: 입력은 SpeechRecognition, 출력은 SpeechSynthesis. **외부 STT 를 쓰지 않는다** —
      제3자 의존 회피(서버를 만든 이유)이자 1코어에서 STT 는 불가능하다.
    🔴 iOS Safari 의 SpeechRecognition 은 지원이 불안정하다 — 안 되면 입력만
      '길게 눌러 녹음 → 텍스트 전사'가 아니라 **텍스트 폴백**으로 조용히 내려간다.
      되는 척하고 아무 반응 없는 것이 최악이다.
    완료 조건: 안드로이드/PC 에서 말로 지시가 들어가고, 비서 답이 소리로 나온다.
      iOS 는 되는지/안 되는지를 실측해 기록한다(추정 금지)

### 【1】 Task 58: 서버 tmux 비서
    파일: scripts/remote/setup-assistant.sh (신규)
    방법: tmux 세션 하나에 claude 상주. **연결 유지가 목적** — 실측상 SSH 새 연결이
      840ms 인데 상주하면 0 이 된다(DB 왕복 114ms 는 무시 가능).
    완료 조건: SSH 를 끊었다 다시 붙어도 같은 세션이 이어진다

### 【1】 Task 59: 서버를 노드로 등록 + 비서 지정 장치
...
### 【1·4】 Task 63: 서버 노드 러너 — 리눅스에서 되는 일만
    파일: scripts/remote/server_runner.py (신규)
    방법: 앱 없이 도는 경량 러너. PTY 대신 tmux send-keys 로 지시.
      **윈도우 전용 작업(빌드·EXE)은 받지 않는다** — 받아서 실패하느니 거절이 낫다.
    완료 조건: 서버 앞으로 발주한 일감이 서버에서 실행되고 결과가 job 에 남는다

### 【2】 Task 64: 결정 대기 푸시 알림
    파일: /var/www/vibe/board/sw.js, collector/push.py
    방법: Web Push(VAPID). 카드가 decide 로 올라올 때만 보낸다.
      🔴 진행 상황을 알리지 않는다 — 알림이 잦으면 꺼버리고, 그러면 정작 결정을 놓친다.
    완료 조건: 폰이 꺼져 있어도 결정 대기 발생 시 알림이 온다

## Phase 12-C — 윈도우 PC 까지 확장 (앱 배포 필요)

### 【4】 Task 65: 앱 배포로 다른 PC 확장
    파일: — (코드는 이미 커밋됨: job_runner·리스너 배선·게이트)
    방법: /vibe-release 로 배포하면 na2js·yjscom 이 일감을 집는다.
    완료 조건: 서버에서 발주한 일감을 3-1 이 실행하고 카드에 결과가 뜬다

### 🔴【4】 Task 66: 세션이 없을 때 새로 띄우기 — "밤에 걸어두기"의 전제
    파일: .ai_monitor/src/job_runner.py, pty-server.js
    방법: 슬롯이 없으면 게이트 통과 확인 후 PTY 스폰(agent=claude|codex, cwd=work_dir).
      끝나면 세션을 정리한다. 지금은 슬롯이 없으면 거부한다 — 즉 **그 PC 에 터미널이
      떠 있어야만 일이 돈다.** 이것이 없으면 무인 운용이 성립하지 않는다.
    🔴 1단계에서 일부러 미룬 항목이다(검증 안 된 게이트 위에서 프로세스를 띄우면
      게이트 버그 하나에 되돌릴 수 없는 실행이 열린다). Task 62·게이트 실사용 후에 한다.
    완료 조건: 터미널이 하나도 없는 PC 에 발주해도 일이 돌고, 끝나면 세션이 사라진다

### 【5】 Task 67: (후순위) 에이전트 간 직접 대화 — 일감 안에 가둔다
    ⚠️ 기본 경로는 **비서 경유**다. 이 Task 는 비서 경유가 실제로 부족하다고
       확인된 뒤에만 착수한다 — 먼저 만들면 종료 안 되는 대화를 떠안는다.
    파일: .ai_monitor/src/pg_jobs.py, job_runner.py
    방법: 일감에 협업 필드(peer_slots, max_turns, goal)를 두고, 그 안에서만 서로 대화한다.
      상한 도달·목표 달성·TTL 초과 중 하나가 오면 **사람에게 보고**하고 멈춘다.
      오간 말은 전부 job_id 로 묶여 이력에 남는다.
    🔴 종료 조건 없는 자율 대화를 만들지 말 것. 창당 상한(_MAX_REMOTE_PER_WINDOW)은
      폭주 방지지 협업 설계가 아니다 — 그것만 믿으면 결론 없이 토큰만 태운다.
    완료 조건: 2-1 과 1-3 이 한 일감 안에서 주고받아 결과를 내고, 상한에 걸리면 멈춘다

### 【4】 Task 68: (선택) 앱에 '결재함 열기'
...
## Phase 12 의존성 (요구 재정의 후)

```
12-A ✅ Task 48 → 49 → 51 → 52 → 53 → 54   (50은 51 전에)

12-B  🔴62(보안 선결) → 55 → 56 → 57(음성) → 64(푸시)
                        55 → 60 → 61
                        58(tmux) → 59(비서 지정) → 63(서버 러너)

12-C  65(배포) → 66(세션 스폰) → 67(자율 협업)
                                  68(선택)
```

**요구 3 은 이미 충족돼 있다** — Task 56 이 기존 /api/projects 를 화면에 붙이는 것으로 끝난다.

## 이 계획이 지키는 것

- **클래식은 손대지 않는다** — 12-B 전체가 앱 무관이다.
- **화면은 한 곳** — btsky.pe.kr/board/. PWA 는 별도 제품이 아니라 그 페이지의 설치 옵션이다.
- **멈춰도 반쪽이 안 남는다** — 12-B 만으로 폰에서 말로 지시하고, 서버가 일하고,
  카드로 결재하는 고리가 완성된다. 12-C 는 윈도우가 필요할 때 붙인다.
- **자율은 가둬서 준다** — 종료 조건 없는 에이전트 대화를 만들지 않는다.

## Phase 12 완료 후 기록할 지식

- `project_apix_jobs_cards` 갱신 — tmux 비서의 수명 관리, 게이트가 실제로 막은 사례
- iOS 음성 인식 실측 결과(되는지/안 되는지) — 추정으로 남기지 말 것
