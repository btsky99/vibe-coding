...
> 자동 생성: `python scripts/generate_project_map.py` | 2026-08-11 08:40
...
- **브랜치**: `main` · 미커밋 2개 · 미푸시 10커밋
- **최근 커밋**
  - `3ee2af1` 2026-08-10 — docs(central): 노드 번호 2번은 결번 — '크립토 PC'는 개발 PC 자신이었다
  - `b488a53` 2026-08-09 — docs(map): 자동 생성 지도 갱신 — central_inject·central_say 신규 파일 반영
...
| `node_identity.py` 🔨 | 222 | 이 PC(노드)의 영구 정체성. config.json에 node_id(uuid4, 최초 1회) + node_label을 |
...
| 컴포넌트 | 줄 수 | 설명 |
|----------|------|------|
| `ActivityBar.tsx` 🔨 | 196 | 설명: 좌측 액티비티 바 — 패널 탭 전환 아이콘 + 배지(태스크/메모리/충돌/Git 변경 수, |
