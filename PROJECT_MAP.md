...
### 1. 코어 서버 (`.ai_monitor/`)
- `server.py`: FastAPI 기반 중앙 통제 서버. 에이전트 로그 중계 및 DB 관리. (v3.6.0: 안정성 패치)
...
- **`vibe-view/`**: React/TypeScript 기반 모니터링 대시보드 프론트엔드. (v3.6.0: 안정성 상태 가시화)
...
- `hive_bridge.py`: 에이전트 작업 로그를 서버로 전송하는 통신 브릿지. (v3.6.0: 인코딩 패치 + 배포 버전 절대경로 수정)
- `hive_watchdog.py`: 시스템 상태 감시 및 자가 복구 엔진. (v3.6.0: 서버 헬스체크 추가)
...
## 🕒 최근 주요 변경 사항
- **[2026-02-28] v3.6.0 배포 버전 경로 버그 수정 (Claude)**:
...
- **[2026-02-28] v3.6.0 패치 완료**:
  - 서버 장기 가동 시 안정성 확보를 위한 소켓 타임아웃 및 SSE 루프 정밀화.
  - 터미널 인코딩 오류(UnicodeEncodeError) 방지를 위해 전역 UTF-8 강제 적용.
  - 워치독(hive_watchdog.py)에 HTTP 서버 헬스체크 및 실시간 상태 모니터링 추가.
