...
> 자동 생성: `python scripts/generate_project_map.py` | 2026-07-30 21:25
...
- **최근 커밋**
  ...
  - `d0fdbd2` 2026-07-29 — docs(plan): CipherTrader 다중 에이전트 설계 + Miix 520 서버 전환 계획
  - `e9954b4` 2026-07-29 — feat(scripts): Termux 노드용 텔레그램-CLI 브릿지 공용화
...
- **07-30 08:39** 의도: list
  - 결정: []
- **07-26 08:09** 의도: Tailnet SSH 풀메시 구축 — desktop-qeuhlak의 CipherTrader 로그를 이 PC 클로드가 직접 읽게 하는 것이 목적
  - 결정: 노드별 키 체계 채택(공용키 아님), 방화벽은 Tailscale 대역 100.64.0.0/10 전용, 키 배포는 Taildrop 경유 스크립트 2단계(setup->addkeys)
  - 다음: 2026-07-28(화) 재개: desktop-qeuhlak과 na2js에서 mesh_addkeys.ps1 실행 + whoami로 계정명 회수 -> ssh config User 교정 -> 양방향 검증 -> CipherTrader 로그 열람
...
| 모듈 | 줄 수 | 설명 |
|------|------|------|
| `_common.py` | 49 | 설명: API 핸들러 공용 헬퍼. 8개 도메인 모듈에 복붙돼 있던 _json_response(8중복)와 |
...
| `hive_api.py` | 1301 | /api/hive/*, /api/orchestrator/*, /api/install-skills, |
...
| `ChatSlot.tsx` 🔨 | 597 | 설명: cokacdir 패턴 채팅 UI 컴포넌트. |
...
| `TerminalSlot.tsx` 🔨 | 1118 | 설명: 하이브 대시보드의 단일 터미널 슬롯 컴포넌트. |
...
| `LanPanel.tsx` 🔨 | 655 | 설명: LAN 브리지 패널 — 같은 네트워크의 다른 바이브코딩을 자동발견하고 페어링(6자리 코드)한 뒤 |
