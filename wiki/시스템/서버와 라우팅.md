...
sources:
  ...
  - .ai_monitor/server.py:338
  - .ai_monitor/server.py:488
  - .ai_monitor/server.py:518
  - .ai_monitor/server.py:539
  - .ai_monitor/server.py:546
  - .ai_monitor/server.py:557
  - .ai_monitor/server.py:593
  - .ai_monitor/server.py:645
  - .ai_monitor/server.py:665
  - .ai_monitor/server.py:734
  - .ai_monitor/server.py:899
  - .ai_monitor/server.py:911
  - .ai_monitor/server.py:920
  - .ai_monitor/server.py:969
  - .ai_monitor/server.py:994
...
> 자동 합성 (코드 주석 57건 · 파일 4개 · 세션 메모리 2장 · 추출 7ca29da).
...
### 🧠 feedback_no_duplicates

**코드 수정 시 중복 패턴 생성 금지. 수정 후 반드시 전체 파일에서 중복 확인. 같은 실수 반복 시 하네스/스킬이 제대로 동작 안 하고 있다는 신호.**

코드 수정 시 기존 코드와 겹치는 패턴을 절대 만들지 말 것. 새 코드를 추가하기 전에 기존 코드를 먼저 확인하고, 겹치면 통합/교체.

**Why:** 반복적으로 같은 필터/핸들러를 다른 위치에 중복 추가하는 실수를 했음. 사용자가 여러 번 지적. 하네스 엔지니어링(스킬, 규칙)이 이런 실수를 잡아줘야 하는데 못 하고 있다는 피드백.

**How to apply:**
1. 파일 수정 전 전체 파일 읽기 필수 (부분만 읽고 추가하면 중복 발생)
2. 수정 후 grep으로 동일 패턴 중복 확인
3. 새 엔드포인트/핸들러 추가 시 → 패스스루/제네릭 방식 우선 고려 (나열 방식 금지)
4. 설계 결정 전 "이거 서버 재시작 필요한가?" 자문 — 필요하면 잘못된 설계

출처: 세션 메모리 `feedback_no_duplicates.md` · type=feedback

### 🧠 feedback_not_browser

**UI 반영/리로드 안내 시 "브라우저 hard reload" "Ctrl+Shift+R" 류 지시 금지. WebView2 내장 창 기준으로 안내할 것**

vibe-coding은 PyWebView + WebView2 기반 **Windows 네이티브 데스크톱 앱**이다. 브라우저에서 열지 않는다. `python .ai_monitor/server.py`는 HTTP 서버 + WebView 창을 함께 띄우는 단일 프로세스다.

**Why:** 반복 실수 패턴. 사용자가 "브라워에서 뛰우는게 아니라고 몇번을 말하니"라고 지적. CLAUDE.md 최상단에도 "Windows PyWebView"로 명시되어 있음에도 프론트엔드 dist 변경 후 "브라우저 Ctrl+Shift+R / hard reload" 같은 잘못된 안내를 반복함.

**How to apply:**
- UI 변경 반영 절차: **앱 프로세스 재시작** (WebView 창 닫고 `python .ai_monitor/server.py` 재실행). 또는 WebView 창에서 **F5 / Ctrl+R** 리로드. "브라우저 열어서…" 금지.
- dist 빌드 후 반영 안내 시: "dev 서버 재시작" 또는 "WebView F5" 라고 정확히 표기.
- 리로드/캐시 관련 조언 전에 "브라우저인가 WebView인가" 먼저 자문할 것.
- localStorage/쿠키 경로도 브라우저 기본이 아닌 `%APPDATA%/vibe-coding/{dev|exe}/…/webview_data/` (project_webview_storage.md 참조).

출처: 세션 메모리 `feedback_not_browser.md` · type=feedback
