...
에러/실패 발생 시:
1. `vibe-heal` 스킬 호출 (1회만)
2. 실패 시 `vibe-debug`로 재시도 (1회만)
3. **⚠️ 하드 리밋: 총 2회 시도 후 무조건 중단. 사용자에게 보고 후 더 이상 자동 재시도하지 않는다.**
4. **절대 금지: 같은 에러에 대해 vibe-heal → vibe-debug → vibe-heal 순환 호출. 한 번 실패한 경로는 재진입하지 않는다.**
5. **자기치유 루프 진입 시 카운터를 반드시 추적하고, 카운터 없이 재시도하는 것을 금지한다.**
