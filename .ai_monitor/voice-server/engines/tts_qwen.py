...
# [🔴 지뢰 — 2026-08-18 실측] 이 기본값은 **혼자서도 사장님이 박으신 못을 넘는다.**
#   0.35 x 12,288MB = 4,301MB 인데 목소리 전체 몫이 3,584MB 다(717MB 초과).
#   그리고 그 못을 지키는 장치가 코드에 없다 — set_per_process_memory_fraction 은
#   **프로세스마다** 걸리는 상한이라(z_qwen_worker.py:175) 합을 아무도 안 막는다.
#   3,584MB 를 몫으로 환산하면 0.2917 이고, 보드와 나눠 쓰려면 각 0.145 여야 한다.
#   [지금 안 터지는 이유] voice_server._bake_only_guard 가 구워진 문장만 이 엔진에 주고
#   나머지는 edge 로 내려보내, **이 사이드카는 일꾼을 아예 안 띄운다.** 그 빗장을 풀거나
#   (VOICE_LOCAL_BAKE_ONLY=0) 누가 무심코 일꾼을 띄우면 **그 순간 못이 깨진다.**
#   [🔴 값을 함부로 내리지 말 것] 왜 0.35 인지(위 주석의 봉우리 근거)를 확인하지 않고
#   내리면 이쪽 일꾼이 먼저 터진다. 내릴 때는 합 기준으로 정하고 보드 쪽과 같이 정해라.
...
VOICES = [
    {
        ...
        'engine': 'qwen',
        'lang': 'ko',
        # [🔴 실측으로 고침 — 2026-08-17] '13초'는 한 문장 굽는 시간이었다. 사장 귀에
        #   닿는 값(/tts/seq 첫 조각 도착)은 예열 상태에서 5.8~23.6초로 글에 따라 크게
        #   흔들린다. 한 숫자로 못 박으면 그것이 곧 거짓말이 된다.
        'note': '미리 구운 문구는 즉시 · 새 문장은 첫 소리까지 6~24초(GPU 상주). 빠른 첫 소리를 원하면 아픽스 나노',
    },
]
