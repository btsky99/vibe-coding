...
DESCRIPTION: edge-tts 낭독 어댑터. 텍스트 → MP3 바이트. 이 앱의 기본 낭독 경로다.
             ...
             [🔴 네트워크에 의존한다 — 실패는 정상 경로다] 인터넷이 끊기거나 MS 가
               막으면 합성이 실패한다. 그때 조용히 기존 경로(브라우저 내장 Heami /
               SAPI / sherpa)로 내려가야 한다. **무음은 어떤 경우에도 안 된다** —
               폴백 판단은 호출부(voice_server.synthesize → 프론트 voiceBus.speak)에 있고,
               여기서는 예외를 그대로 올리는 것이 계약이다(삼키면 폴백이 안 돈다).
