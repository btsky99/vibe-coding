...
#   2026-08-14 실측(표본 9건, 지식 5169건):
#     관련 최저 0.860 / 무관 최고 0.847 → 간격 +0.013. 그래서 0.85였다.
#   2026-08-15 재측정(W8 — 위키 전환 후, **표본 24건**으로 확장, 인덱스 1021 섹션):
#     관련 최저 0.872 (12/12 통과) / 무관 최고 0.850 → 간격 +0.022.
#     0.85 로는 "오늘 점심 뭐 먹지"가 0.850 으로 **경계를 넘어** 통과했다 —
#     옛 값의 여유가 0.003 뿐이었고, 위키가 인덱스에 들어오며 그 3천분의 3이 사라졌다.
#     후보 표: 0.86·0.87 이 관련 12/12 · 무관 0/12. 0.87 은 관련 최저와 0.002 밖에
#     안 떨어져 **0.86** 을 고른다(위 0.012 / 아래 0.010 대칭).
#
# [🔴 이 값의 한계를 알고 쓸 것 — 여전히 표본에 맞춘 숫자다]
#   표본을 9 → 24건으로 늘려 '우연히 맞은 값'일 확률은 줄었지만, 여유는 여전히
#   ±0.01 대다. **표본 밖 질의 하나로 또 뒤집힐 수 있다**(2026-08-15 에 실제로 뒤집혔다).
