 ...
 * ------------------------------------------------------------------------
 * 📄 파일명: ActivityBar.tsx
 * 📝 설명: VS Code 스타일 좌측 아이콘 액티비티 바 컴포넌트.
 *          각 탭(탐색기, 검색, 오케스트레이터, 하이브, 메시지, 태스크,
 *          메모리, Git, MCP, 스킬결과)으로 전환하는 버튼들과
...
interface ActivityBarProps {
  ...
  orchWarningCount: number;    // 하이브 탭 경고 배지
  unreadMsgCount: number;      // 메시지 탭 미읽음 배지
  activeTaskCount: number;     // 태스크 탭 진행 중 배지
...
export default function ActivityBar({
  ...
  // 탭 버튼 공통 스타일 계산 — 활성 탭은 왼쪽 보더 + 배경 강조
  ...
  return (
    <div className="w-12 h-full bg-[#333333] border-r border-black/40 flex flex-col items-center py-4 gap-4 shrink-0">
      ...
      {/* ── 태스크 보드 — 진행 중 태스크 수 배지 ── */}
      <button onClick={() => handleTab('tasks')} className={tabCls('tasks')} title="태스크 보드">
