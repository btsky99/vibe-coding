 ...
 * 📝 설명: 오케스트레이션 모니터 보드 — 상단 통계(활성 터미널/실행 중 단계/최근 완료),
 *          활성 터미널 카드(라이브 스킬 체인), 최근 완료 세션. 3초 폴링:
 *          /api/orchestrator/skill-chain + /api/agent/terminals + /api/skill-results.
 * REVISION HISTORY:
 * - 2026-07-15 Claude: 유령 조직도 절제 — agent_heartbeats 무필터 덤프로 11~73일 전
 *                      죽은 행이 영구 표시되고, '카드 클릭 재시도'는 NOTIFY task_assigned의
 *                      유일한 리스너(hive_heartbeat.py)가 어느 시동 경로에도 없어 완전 무기능.
 *                      Dispatcher 카드는 2차 정리(e42ff2f)로 백엔드가 사라진 잔재.
 *                      실체 있는 체인 추적판(통계+활성 터미널+최근 완료)만 유지.
 * ------------------------------------------------------------------------
 */
