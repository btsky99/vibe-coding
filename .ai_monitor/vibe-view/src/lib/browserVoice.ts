 ...
 *
 *          [🔴 이 파일이 낭독의 마지막 방어선이다 — 2026-08-15 엔진 정리] 사이드카의
 *          로컬 엔진(sherpa VITS·SAPI)을 걷어내 그쪽에 남은 것은 edge-tts 하나뿐이고,
 *          edge 는 네트워크에 기댄다. 망이 끊기거나 MS 가 막히면 **여기가 유일하게
 *          소리를 내는 곳**이다. 지우거나 '안 쓰는 코드'로 오해하지 말 것 —
 *          이 스택에서 가장 나쁜 결과는 느린 것이 아니라 무음이다.
 *
 *          [WHY 이 길이 안 죽나] 프로세스도 모델도 네트워크도 없다. 그래서 **준비
 *          상태라는 개념 자체가 없다** — 사이드카가 통째로 죽어도 이건 말한다.
 *          호출 순서는 voiceBus.speak 이 정한다(지금은 사이드카 → 여기 순).
 *
 ...
 * - 2026-08-15 Claude: 낭독 엔진 정리로 이 경로가 유일한 폴백이 됨 — 머리말 갱신
 * ------------------------------------------------------------------------
 */
...
/** 브라우저 목소리 id 앞에 붙는 표시. 사이드카 목소리('edge:')와 섞이지 않게 한다. */
