...
INTENT_MAP = [
    {
        ...
        "keywords": [
            ...
            "codex", "코덱스",
            "클로드가", "안티그라비티가", "안티그라비티가", "코덱스가",
            "각자", "각각", "나눠서", "분담해서",
            "지시해", "시켜", "맡겨", "delegate",
        ],
        # [2026-06-11] auto_dispatcher.py 폐기(5차 정리)로 안내 교체 —
        # 폐기된 스크립트 실행을 지시하던 죽은 배너가 매 세션 주입되던 문제
        "context": (
            "═══════════════════════════════════════════════════════\n"
            ...
            "2) 태스크 큐 등록: python scripts/task.py create \"내용\" --agent <대상>\n"
            "3) 터미널 직접 지시: python scripts/send_message.py (ITCP)\n"
            "4) 진행 상황은 hive_tasks 테이블로 공유됨\n"
            "\n"
            "⚠️  복합 워크플로면 /vibe-orchestrate 스킬을 우선 고려\n"
            "═══════════════════════════════════════════════════════"
        ),
    },
    {
