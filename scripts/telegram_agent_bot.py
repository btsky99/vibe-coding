# -*- coding: utf-8 -*-
"""
FILE: scripts/telegram_agent_bot.py
DESCRIPTION: AgentBot — 터미널 1개 = 텔레그램 봇 1개. 개인채팅 1:1 PTY/stream-json
             스트리밍 + 그룹채팅 발화를 담당. telegram_bridge.py(BotManager/main)에서
             import하여 사용 — 단독 실행 진입점 아님.

             [소유 전역 — 분할 불변식]
             TERMINAL_CLI_MAP / GROUP_CHAT_ID는 이 모듈이 소유한다.
             telegram_bridge.py(BotManager)는 반드시 `telegram_agent_bot.TERMINAL_CLI_MAP = ...`
             모듈 속성 경유로 갱신할 것 — `from ... import`로 가져가면 스냅샷 바인딩이라
             갱신이 AgentBot에 반영되지 않는다.

REVISION HISTORY:
- 2026-07-16 Claude: telegram_bridge.py(1455줄, 권장 분할선 1200 초과)에서 AgentBot
  클래스 + CLI 매핑/그룹ID 전역 분리 (1500줄 규칙 §2 예방 분할).
"""
...
# 로거는 telegram_bridge.py의 basicConfig 설정을 그대로 공유 (같은 이름)
log = logging.getLogger("telegram_bridge")

# ── .env 로드 ──
# [제약] GROUP_CHAT_ID를 아래에서 env로 읽으므로 이 모듈 상단에서 dotenv 로드 필수
# (telegram_bridge.py 로드 순서에 의존하지 않도록 자체 로드 — load_dotenv는 멱등)
try:
