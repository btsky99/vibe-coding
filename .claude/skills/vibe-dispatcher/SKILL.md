...
description: >
  ...
  Use when: "태스크 분배", "에이전트 배정", "작업 보내줘", "T2한테 시켜", "디스패치", "팬아웃",
  "에이전트 상태", "누가 뭐 하고 있어", "검증 요청", "크로스 리뷰", "멀티 에이전트",
  "Gemini한테", "Codex한테", "병렬 작업", "작업 분배해줘", "하이브 상태" 요청 시.
  단일 에이전트로 충분한 작업이라도 사용자가 특정 에이전트를 지명하면 이 스킬을 사용하세요.
allowed-tools: Read, Write, Edit, Bash, Glob, Grep
user-invocable: true
---

<!--
FILE: .claude/skills/vibe-dispatcher/SKILL.md
DESCRIPTION: Claude용 멀티 LLM 디스패처 관리 스킬.
             auto_dispatcher.py CLI를 통해 하이브 에이전트 간 태스크 분배/모니터링/검증을 수행.
...
> 상세 옵션은 `$CLAUDE_SKILL_DIR/references/dispatcher-commands.md` 참조
...
```bash
python scripts/auto_dispatcher.py report-verify TASK_ID "검증 의견" --verdict approved --reviewer claude
```
...
📋 유형: security (자동 감지)
🎯 할당: Claude (T1) — 적합도 0.92
🔍 검증: Gemini (T2)가 크로스 검증 예정
⚡ 우선순위: high
...
📡 Fan-Out 분배 완료 (3건)
━━━━━━━━━━━━━━━━━━━━━━━━━
1. 보안 점검     → Claude (T1)  [security, 0.92]
2. 성능 테스트   → Codex (T3)   [perf, 0.84]
3. UI 리뷰      → Gemini (T2)  [frontend, 0.78]
━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
| 작업 | 최적 에이전트 | 이유 |
|------|-------------|------|
| 코드 리뷰, 보안 분석 | **Claude (나)** | security 0.95, code_review 0.95 |
| 웹 조사, 트렌드 분석 | **Gemini** | research 0.95 (나는 0.30) |
| 아키텍처 설계 | **Gemini** | architecture 0.90 |
| 빠른 코드 생성, 테스트 | **Codex** | testing 0.90, 샌드박스 병렬 실행 |
| 문서화 | **Gemini** | documentation 0.90 |
| 디버깅 | **Claude (나)** | debugging 0.90, precision_logic 0.90 |

> 내가 잘하는 작업(보안, 리뷰, 디버깅)은 직접 수행하되, 검증만 다른 에이전트에 요청한다.
> 내가 약한 작업(웹 검색, 광범위 조사)은 적극적으로 Gemini/Codex에 위임한다.
...
- 사용자가 "T2한테 보내" 같이 터미널 번호로 지정하면: T1=claude, T2=gemini, T3=codex.
