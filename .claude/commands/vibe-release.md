...
# Step 2: PyInstaller EXE 패키징
cd ../../ && pyinstaller vibe-coding.spec --noconfirm
# → dist/vibe-coding-vX.Y.Z.exe 생성

# Step 3: Inno Setup 설치버전 빌드
# ⚠️ vibe-coding-setup.iss 의 MyAppVersion을 _version.py와 반드시 동기화!
...
> ⚠️ Step 2 전에 vibe-coding-setup.iss의 `#define MyAppVersion`을 _version.py와 맞춰야 함.
> ⚠️ ISS의 `MyAppPublisher`는 반드시 `"Vibe Coding Team"`으로 유지할 것.
