...
## 🔴 스킬과 무관하게 상시 쓰는 팀원 — `check`

| Subagent | model | 넘기는 것 | 🔴 안 넘기는 것 |
|---|---|---|---|
| `check` | **sonnet** | 세기 · 찾기 · 파일 뒤지기 · 표 만들기 · 재기 · 로그 훑기 · 되풀이 | 원인 규명 · 설계 · 판정 · 커밋 · 배포 |

위 3개와 달리 특정 스킬에 묶이지 않는다. **아무 작업 중에나 부른다.**

[WHY] 이것이 없어서 팀장이 최상급 모델로 파일을 세고 있었다(2026-08-19 사장 지적:
"왜 혼자 일하고 있니. 직원들 안 시키고"). 🔴 `check.md` 의 `model: sonnet` 을 지우면
부모 모델을 상속해 **조용히 비싸진다** — 아무도 눈치채지 못하므로 지우지 마라.
